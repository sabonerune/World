...
static void GetWindowedWaveform(const double *x, int x_length, int fs,
    ...
    double window_length_ratio, double *waveform, wmi::RandomGenerator &wmi_rand) {
  ...
  for (int i = 0; i <= half_window_length * 2; ++i)
    waveform[i] =
      x[safe_index[i]] * window[i] + wmi_rand() * world::kSafeGuardD4C;
...
static void GetCentroid(const double *x, int x_length, int fs,
    ...
    const ForwardRealFFT *forward_real_fft, double *centroid, wmi::RandomGenerator &wmi_rand) {
  for (int i = 0; i < fft_size; ++i) forward_real_fft->waveform[i] = 0.0;
  GetWindowedWaveform(x, x_length, fs, current_f0,
      current_position, world::kBlackman, 4.0, forward_real_fft->waveform, wmi_rand);
  double power = 0.0;
...
static void GetStaticCentroid(const double *x, int x_length, int fs,
    double current_f0, int fft_size, double current_position,
    const ForwardRealFFT *forward_real_fft, double *static_centroid, wmi::RandomGenerator &wmi_rand) {
  ...
  GetCentroid(x, x_length, fs, current_f0, fft_size,
      current_position - 0.25 / current_f0, forward_real_fft, centroid1, wmi_rand);
  GetCentroid(x, x_length, fs, current_f0, fft_size,
      current_position + 0.25 / current_f0, forward_real_fft, centroid2, wmi_rand);
...
static void GetSmoothedPowerSpectrum(const double *x, int x_length, int fs,
    double current_f0, int fft_size, double current_position,
    const ForwardRealFFT *forward_real_fft, double *smoothed_power_spectrum, wmi::RandomGenerator &wmi_rand) {
  for (int i = 0; i < fft_size; ++i) forward_real_fft->waveform[i] = 0.0;
  GetWindowedWaveform(x, x_length, fs, current_f0,
      current_position, world::kHanning, 4.0, forward_real_fft->waveform,wmi_rand);
...
static double D4CLoveTrainSub(const double *x, int fs, int x_length,
    ...
    ForwardRealFFT *forward_real_fft, wmi::RandomGenerator &wmi_rand) {
  ...
  GetWindowedWaveform(x, x_length, fs, current_f0, current_position,
    world::kBlackman, 3.0, forward_real_fft->waveform, wmi_rand);
...
static void D4CLoveTrain(const double *x, int fs, int x_length,
    ...
    double *aperiodicity0, wmi::RandomGenerator &wmi_rand) {
  ...
  for (int i = 0; i < f0_length; ++i) {
    ...
    aperiodicity0[i] = D4CLoveTrainSub(x, fs, x_length,
      ...
      fft_size, boundary0, boundary1, boundary2, &forward_real_fft, wmi_rand);
  }
...
static void D4CGeneralBody(const double *x, int x_length, int fs,
    ...
    const ForwardRealFFT *forward_real_fft, double *coarse_aperiodicity, wmi::RandomGenerator &wmi_rand) {
  ...
  GetStaticCentroid(x, x_length, fs, current_f0, fft_size, current_position,
      forward_real_fft, static_centroid, wmi_rand);
  GetSmoothedPowerSpectrum(x, x_length, fs, current_f0, fft_size,
      current_position, forward_real_fft, smoothed_power_spectrum, wmi_rand);
...
void D4C(const double *x, int x_length, int fs,
  ...
  for (int i = 0; i < f0_length; ++i) {
    ...
    D4CGeneralBody(x, x_length, fs, MyMaxDouble(world::kFloorF0D4C, f0[i]),
        ...
        window_length, &forward_real_fft, &coarse_aperiodicity[1], wmi_rand);
